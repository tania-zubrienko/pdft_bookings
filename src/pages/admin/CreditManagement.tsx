...
export default function CreditManagement() {
    ...
    useEffect(() => {
        Promise.all([getStudents(), getPackages()]).then(([studentsRes, packagesRes]) => {
    ...
    useEffect(() => {
        ...
        getCreditPoolsByStudent(selectedStudentId).then(setPools);
    }, [selectedStudentId]);
    ...
    const submit = async (e: FormEvent) => {
        ...
        if (start >= end) {
            setFormError('La fecha de inicio debe ser anterior a la fecha de expiración.');
            return;
        }
        ...
        try {
            ...
            const refreshed = await getCreditPoolsByStudent(selectedStudentId);
            setPools(refreshed);
            setNotes('');
        ...
        <AdminLayout>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
                <section className='card p-5 lg:col-span-1'>
                    <h2 className='text-lg font-semibold text-gray-100 mb-3'>Seleccionar alumna</h2>
                    ...
                    <div className='mt-4 space-y-2 text-sm text-gray-300'>
                        <p>
                            <span className='text-gray-400'>Alumna:</span> {selectedStudent?.name ?? '—'}
                        </p>
                        <p>
                            <span className='text-gray-400'>Créditos activos:</span> {stats.activeCredits}
                        </p>
                        <p>
                            <span className='text-gray-400'>Pools por vencer ({EXPIRING_SOON_DAYS} días):</span>{' '}
                            {stats.expiringSoon}
                        </p>
                    </div>
                </section>
                ...
                <section className='card p-5 lg:col-span-2'>
                    <h2 className='text-lg font-semibold text-gray-100 mb-4'>Crear pool de créditos</h2>
                    ...
                    <form onSubmit={submit} className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                        <div>
                            <label className='block text-sm text-gray-300 mb-1'>Créditos</label>
                            <input
                                type='number'
                                min={1}
                                step={1}
                        ...
                        <div>
                            <label className='block text-sm text-gray-300 mb-1'>Referencia de paquete (opcional)</label>
                            <select
                                className='input'
                                value={packageId}
                        ...
                        <div>
                            <label className='block text-sm text-gray-300 mb-1'>Fecha de expiración</label>
                            <input
                                type='date'
                                className='input'
                                value={expiresAt}
            ...
            <section className='card p-5 mt-6'>
                <h2 className='text-lg font-semibold text-gray-100 mb-4'>Pools de créditos</h2>

                {pools.length === 0 ? (
                    <p className='text-gray-400 text-sm'>No hay pools para la alumna seleccionada.</p>
                ) : (
                                ...
                                >
                                    <div className='text-sm text-gray-200'>
                                        ...
                                        <p className='text-gray-400'>
                                            Válido: {pool.startDate.toLocaleDateString('es-ES')} → {pool.expiresAt.toLocaleDateString('es-ES')}
                                        </p>
                                        {pool.packageId && <p className='text-gray-400'>Paquete: {pool.packageId}</p>}
                                        {pool.notes && <p className='text-gray-400'>Notas: {pool.notes}</p>}
                                    </div>
